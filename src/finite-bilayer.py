...
def Hamiltonian_Heisen_In_Double_Trian(J1,J2,J3,Hz,spin):
    ...
    H_AB = np.kron(np.kron(Sx,Sx), I) + np.kron(np.kron(Sy,Sy), I) + np.kron(np.kron(Sz,Sz), I)
    H_CA = np.kron(np.kron(Sx,I), Sx) + np.kron(np.kron(Sy,I), Sy) + np.kron(np.kron(Sz,I), Sz)
    H_on = ( np.kron(Sx,Sx) + np.kron(Sy,Sy)+ np.kron(Sz,Sz)).reshape(d_spin, d_spin, d_spin, d_spin)
    I4 =np.eye(d_spin**4, d_spin**4).reshape(d_spin, d_spin, d_spin ,d_spin, d_spin, d_spin, d_spin, d_spin)

    H_on = np.einsum('ijab,klmncdef -> ijklmnabcdef', H_on, I4)

    H_tri =  (H_AB + H_BC + H_CA).reshape(d_spin, d_spin, d_spin, d_spin, d_spin, d_spin)
    I3 = np.eye(d_spin**3, d_spin**3).reshape(d_spin, d_spin, d_spin, d_spin, d_spin, d_spin)

    expr_kron = 'ijkabc,lmndef -> ijklmnabcdef'
    H_tri= np.einsum(expr_kron, H_tri, I3)

    #print(np.real(H_tri.transpose(0,3,1,4,2,5,6,9,7,10,8,11).reshape(64,64)))

    #### test ####
    """
    H_test =    np.kron(Sx, np.kron(Sx, np.kron(I, np.kron(I, np.kron(I, I))))) +\
                np.kron(Sy, np.kron(Sy, np.kron(I, np.kron(I, np.kron(I, I))))) +\
                np.kron(Sz, np.kron(Sz, np.kron(I, np.kron(I, np.kron(I, I))))) +\
                np.kron(I, np.kron(I, np.kron(Sx, np.kron(Sx, np.kron(I, I))))) +\
                np.kron(I, np.kron(I, np.kron(Sy, np.kron(Sy, np.kron(I, I))))) +\
                np.kron(I, np.kron(I, np.kron(Sz, np.kron(Sz, np.kron(I, I))))) +\
                np.kron(I, np.kron(I, np.kron(I, np.kron(I, np.kron(Sx, Sx))))) +\
                np.kron(I, np.kron(I, np.kron(I, np.kron(I, np.kron(Sy, Sy))))) +\
                np.kron(I, np.kron(I, np.kron(I, np.kron(I, np.kron(Sz, Sz)))))

    print(np.real(H_test) - ( H_on + H_on.transpose(2,3,0,1,4,5,8,9,6,7,10,11) + H_on.transpose(2,3,4,5,0,1,8,9,10,11,6,7) ).reshape(64,64) )
    """
    ##############

    H_tri = J1*H_tri.transpose(0,3,1,4,2,5,6,9,7,10,8,11) + J2*H_tri.transpose(3,0,4,1,5,2,9,6,10,7,11,8) +\
            0.5*J3*( H_on + H_on.transpose(2,3,0,1,4,5,8,9,6,7,10,11) + H_on.transpose(2,3,4,5,0,1,8,9,10,11,6,7))

    H_tri = H_tri.reshape(d_spin**2 * d_spin**2 * d_spin**2, d_spin**2 * d_spin**2 * d_spin**2)


    return np.real(H_tri)
###########################################################################
